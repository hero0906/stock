...
#        self.params.extend(['s_sh000001', 's_sz399001'])
    ...
    def value_get(cls, code, code_index):
        ...
        res = r.text.split(',')
        if len(res) > 1:
            name, start, now, high, low, tom_stop = r.text.split(',')[0][slice_num:], r.text.split(',')[1], r.text.split(',')[value_num], r.text.split(',')[4],\
            r.text.split(',')[5], r.text.split(',')[2]
#            format(float(-2)/float(19),'.2f')
            percent = format((float(now) - float(start))/float(start)*100,'.3f')
            tom_now_percent = format((float(now) - float(tom_stop))/float(tom_stop)*100,'.3f')
        return code_index, pin.get_pinyin(name)[0] + ' ' + start + ' ' + now + ' ' + high + ' ' + low\
            +' ' + tom_now_percent + '% '
...
if __name__ == '__main__':
    ...
    parser.add_option('-s', '--sleep-time', dest='sleep_time', default=2, type="int",
